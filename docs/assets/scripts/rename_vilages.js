...
async function renameVillages() {
  ...
  var rename_links = document.querySelectorAll('a.rename-icon[data-title="Renomear"]');
  var new_name = document.querySelector("#rename-div #my-input-text").value;

  for (let link of rename_links) {
    link.click();
    await delay(200);
    var rename_input = document.querySelector('input[type="text"]:not(#my-input-text)');
    var rename_button = document.querySelector('input[type="button"]:not(#my-input-button)');
