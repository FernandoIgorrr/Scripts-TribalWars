...
// Modificado by: Black Disciple/Black Magic/Show no Mercy
      ...
      for (let prop in plan) {
        ...
        plan[prop].forEach((val, i) => {
          html += `<tr class="farmRow row_${(i % 2 == 0) ? 'a' : 'b'}">
                    ...
                    <td style="text-align:center;"><a name="btn" href="#" data-origin="${val.origin.id}" data-target="${val.target.id}" data-template="${val.template.id}" class="farmGod_icon farm_icon farm_icon_${val.template.name}" style="margin:auto;"></a></td>
                  </tr>`;
        });
      }
    } else {
